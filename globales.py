...
def aleatorio():
    ...
    for nombre in objetos:
        ...
        productos.append(valorasignado)
    guardar_archivo_json('productos.json', productos)
...
def buscar(producto):
    ...
    if producto == "bajo":
        productos = leer_archivo_json('productos.json')
        productos_ordenados = (productos).sorted, key = lambda x: x ["iva"], reversed = False
...
def promedio():
    ...
    for producto in productos:
        productos_totales += 1
        valor_total += producto["iva"]
    
    promedio = valor_total / productos_totales
...
def geometrica():
    ...
    for producto in productos:
        productos_totales += 1
        valor_total += math.log(producto["iva"])
    
    geometrica = math.exp(valor_total / productos_totales)
